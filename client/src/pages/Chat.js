...
function Chat({ user, receiver }) {
  ...
  const API_URL = process.env.REACT_APP_API_URL || "https://chat-app-simran.onrender.com";

  const fetchMessages = async () => {
    if (!receiver) return;
    try {
      const res = await axios.get(`${API_URL}/messages/${user.id}/${receiver.id}`);
  ...
  const sendMessage = async () => {
    ...
    setText("");
    try {
      await axios.post(`${API_URL}/messages`, msg);
    } catch (err) {
